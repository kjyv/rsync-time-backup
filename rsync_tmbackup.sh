...
fn_rm_dir() {
    #fn_run_cmd "rm -rf -- '$1'"
    fn_mkdir "/tmp/rsync-time-backup-emptydir"
    fn_run_cmd "rsync -r --delete /tmp/rsync-time-backup-emptydir/ '$1'"
    fn_run_cmd "rmdir /tmp/rsync-time-backup-emptydir '$1'"
}
...
<<<<<<< HEAD
    ...
    EXIT_CODE="1"
    if [ -n "$(grep "rsync error:" "$LOG_FILE" | grep -v "some files/attrs were not transferred")" ]; then
        ...
        fn_log_warn "Rsync reported one or more warnings. Run this command for more details: grep -E 'rsync:|rsync error:' '$LOG_FILE'"
        EXIT_CODE="0"
    else
